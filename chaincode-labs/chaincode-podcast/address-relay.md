...
today we want to hear a little bit more about your background, sort of how you got to working on Bitcoin Core, and then some of the things you're working on.
...
I'm going to change this now, because this might lead to changes that are not
...
Right.
That is something that is, I think in Lightning Network, it's being published, but at least for the public nodes and in Bitcoin Core, this is something that nobody should be able to get this information
...
And that is something I find really
...
And that is how nodes self-advertisements
...
## Correction: The peer would not get addresses-divided-by-peers addresses, but 2x addresses-divided-by-peers addresses as the addresses get forwarded to two peers each.
10 peers connected to the attack node, they'll receive 500.
...
And they actually have a note that monitors
...
the whole network and probably put the link there in the show notes.
...
And so I think this is something, I don't know if the attacker wanted to know this, but it's definitely something that the Bitcoin research community can make use
...
Well, if we have some errors in the try table, if we don't have done we are
...
Like, getAdder messages were taken advantage of by,

## Coinscope paper

in the Coinscope paper, where they used specific timestamps that were proliferated through the network to understand network topology.
